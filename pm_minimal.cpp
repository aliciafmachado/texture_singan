/* -------------------------------------------------------------------------
  SinGAN project: This is the official minimal implementation of patchmatch, which was slightly modified to save the
  data as we want. So, the code where we don't indicate is ours, it's from the author. By us, we mean
  Alicia FORTES MACHADO and Raphael MENDES.
...
// Modifications from SinGAN project for Imagerie Numerique --------------
#include <iostream>
#include <fstream>
using namespace std;
// -----------------------------------------------------------------------
...
// Modifications from SinGAN project of Imagerie Numerique ---------------------------------
void save_arr_to_file(const char *filename, BITMAP *bmp, int f_type) {
  // if f_type == 0, we are dealing with ann
  // if f_type == 1, we are dealing with annd
  int w = bmp->w;
  int h = bmp->h;
  ...
  if (myfile.is_open())
  {
      for(int i = 0; i < h; i++) {
        for(int j = 0; j < w; j++) {
          if(f_type == 0) {
            myfile << INT_TO_X((*bmp)[i][j]) << " " ;
            myfile << INT_TO_Y((*bmp)[i][j]) << " " ;
          }
          else {
            myfile << (*bmp)[i][j] << " " ;
          }
...
int main(int argc, char *argv[]) {
  ...
  // Modifications - SinGAN project of Imagerie Numerique ---------
  printf("Saving arrays\n");
  save_arr_to_file("ann.txt", ann, 0);
  save_arr_to_file("annd.txt", annd, 1);
  // --------------------------------------------------------------
